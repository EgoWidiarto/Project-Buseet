...
$schedule = mysqli_query($conn, "SELECT * FROM sopir LIMIT 10;");

if (isset($_POST["search"])) {
    $search = mysqli_real_escape_string($conn, $_POST["driver-search"]);
    $schedule = mysqli_query($conn, "SELECT * FROM sopir WHERE nama_lengkap LIKE '%$search%' OR NIP LIKE '%$search%'");
}
?>
       ...
       <form action="" method="post">
        <div class="d-flex">
          ...
          <div class="input-group mb-3">
            ...
            <input type="text" class="form-control curved" name="driver-search" placeholder="Cari Sopir" aria-label="Pencarian Sopir" aria-describedby="search-driver-data" required />
          </div>
        </div>
        <button class="btn btn-outline-primary btn-light btn-lg" type="submit" name="search" aria-label="Cari Sopir">Cari Sopir</button>
      </form>
    </section>
    ...
    <section class="list-bus text-center">
      <div class="mt-4">
        <div class="mb-2 text-end" style="margin-right: 6.5rem">
          <a href="driver-add.php" role="button" class="btn bus-add-btn text-black"><i class="bi bi-plus-square-dotted me-2"></i>Tambah Sopir</a>
        </div>
        ...
        <tbody>
            ...
            <tr>
                <td><?= $num; ?></td>
                <td class="font-15"><?= $row["NIP"]; ?></td>
                <td class="font-15"><?= $row["nama_lengkap"]; ?></td>
                <td><?= $row["alamat"]; ?></td>
                <td><?= $row["nomor_hp"]; ?></td>
                <td>
                    <a href="../config/admin-bus-delete.php?driver-id=<?= $row['NIP'] ?>" class="text-decoration-none text-danger"><i class="bi bi-trash3-fill"></i>Hapus</a>
                </td>
            </tr>
            <?php $num++; ?>
            <?php  endwhile; ?>
