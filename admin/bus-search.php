...
$schedule = mysqli_query($conn, "SELECT * FROM bus LIMIT 10;");

if (isset($_POST["search"])) {
    $search = mysqli_real_escape_string($conn, $_POST["bus-search"]);
    $schedule = mysqli_query($conn, "SELECT * FROM bus WHERE nama_bus LIKE '%$search%'");
}
?>
       ...
       <form action="" method="post">
        <div class="d-flex">
          ...
          <div class="input-group mb-3">
            ...
            <input type="text" class="form-control curved" name="bus-search" placeholder="Cari Bus" aria-label="Bus" aria-describedby="input-bus" required />
          </div>
        </div>
        <button class="btn btn-outline-primary btn-light btn-lg" type="submit" name="search" aria-label="Cari Bus">Cari Bus</button>
      </form>
    </section>
    ...
    <section class="list-bus text-center">
      <div class="mt-4">
        <div class="mb-2 text-end" style="margin-right: 6.5rem">
          <a href="bus-add.php" role="button" class="btn bus-add-btn text-black"><i class="bi bi-plus-square-dotted me-2"></i>Tambah Bus</a>
        </div>
        ...
        <tbody>
            ...
            <tr>
                <td><?= $num; ?></td>
                <td class="font-15"><?= $row["nama_bus"]; ?></td>
                <td class="font-15"><?= $row["tipe_bus"]; ?></td>
                <td><?= $row["kapasitas"]; ?></td>
                <td>
                    <a href="../config/admin-bus-delete.php?bus-id=<?= $row['Id_bus'] ?>" class="text-decoration-none text-danger"><i class="bi bi-trash3-fill"></i>Hapus</a>
                </td>
            </tr>
            <?php $num++; ?>
            <?php  endwhile; ?>
